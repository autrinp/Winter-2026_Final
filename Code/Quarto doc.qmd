...
```{r}
mode_null <- lm(used_bin ~ 1,
             data = mopl_clean)

```

```{r}
mod_sat <- lm(used_bin ~ edgedist + vo_nest, # making new object mod_sat, both edgedist and vo as predictors
               data = mopl_clean) # pulling from data set
```

```{r}
mod_edgdist <- lm(used_bin ~ edgedist, # making new object mod_edge, edgedist as predictor
                data = mopl_clean)# data
```

```{r}
mod_vo <- lm(used_bin ~ vo_nest, # making new object mod_vo, vo as predictor
              data = mopl_clean) # data
```
