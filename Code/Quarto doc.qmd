...
My_data_clean <- my_data[-1,] |> # creating new data set 
  ...
  select(flips_bin, biking_time_mins) #selecting only binary response and predictor
```

```{r}
#| message: false
my_mod <- glm(flips_bin ~ biking_time_mins, # creating a glm, ocean_bin as a function of workload
...
```{r}
#| message: false
plot(simulateResiduals(my_mod) # using DHARMa to plot residuals
)
```
